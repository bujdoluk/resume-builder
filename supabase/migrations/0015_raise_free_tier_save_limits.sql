...
create policy "Users can insert their own resumes" on public.resumes
  for insert with check (
    ...
    and (
      ...
      or (select count(*) from public.resumes where resumes.user_id = auth.uid()) < 10
    )
  );
...
create policy "Users can insert their own cover letters" on public.cover_letters
  for insert with check (
    ...
    and (
      ...
      or (select count(*) from public.cover_letters where cover_letters.user_id = auth.uid()) < 10
    )
  );

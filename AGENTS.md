...
├── supabase/migrations/            # 13 numbered SQL migrations, applied to production by prod.yml's `migrate` job
...
├── .github/workflows/              # dev.yml, prod.yml — typecheck, lint, npm audit, test (prod.yml also builds, then on
│                                    # a push to main — not a PR — applies pending Supabase migrations and deploys to
│                                    # Vercel via `vercel deploy --prod`, gated on the checks and migration succeeding);
...
- Deployment is already automatic (`prod.yml`'s `deploy` job runs `vercel deploy --prod` on every push to `main`, after migrations succeed) — releasing is purely about generating a changelog and tagging what's live, not shipping.

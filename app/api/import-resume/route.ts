...
export async function POST(request: Request) {
  ...
  let text: string;
  try {
    text = await extractResumeText(fileBuffer, fileType);
  } catch (error) {
    // Always logged (including the wrapped cause) — extractResumeText
    // rewraps every extraction failure into ResumeImportExtractionError, so
    // without this the underlying pdfjs-dist/mammoth error would never
    // surface anywhere.
    console.error(error, error instanceof ResumeImportExtractionError ? error.cause : undefined);
    if (!(error instanceof ResumeImportExtractionError)) {

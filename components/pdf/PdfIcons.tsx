/**
 * Every icon used inside the generated PDF (contact fields, section
 * headers, language star rating), redrawn with `@react-pdf/renderer`'s own
 * Svg/Path/Circle/Rect primitives since it can't render a plain DOM `<svg>`.
 * See `components/Icons.tsx` for the on-screen equivalents this mirrors.
 */
import { Circle, Path, Rect, Svg } from "@react-pdf/renderer";

// Mirrors components/Icons.tsx's path data exactly, redrawn with
// @react-pdf/renderer's own SVG primitives — react-pdf can't render a plain
// DOM <svg>, it needs its own Svg/Path/Circle/Rect components.
...
// The rating widget draws real star shapes rather than Unicode ★/☆
// characters — the subsetted Google Fonts webfont files registered for the
// resume's typography only cover Latin glyphs, so star characters silently
// fail to render (no glyph, no fallback) in the generated PDF.
//
// daisyUI's own .rating renders every star as the *same* solid color, only
// varying opacity (unchecked: 0.2, checked: 1) — see node_modules/daisyui/
// components/rating.css (`& * { background-color: var(--color-base-content);
// opacity: .2 }` + `& :checked { opacity: 1 }`). It's not an outline-vs-fill
// distinction, so this mirrors that exactly rather than approximating it.
const STAR_PATH =

...
export default function ClassicMobileTemplate({
  data,
  onChange,
  onWorkHistoryChange,
  onEducationChange,
    ...
    >
      <div className="h-24 w-24 rounded-full bg-white">
        {data.photo ? (
          ...
          <div className="text-neutral flex h-full w-full flex-col items-center justify-center gap-1">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              fill="none"
              viewBox="0 0 24 24"
  ...
  const jobTitle = !visibleFields.includes("jobTitle") ? null : (
    <fieldset className="fieldset">
      <input
        ...
        className="input input-plain w-full text-lg"
        value={data.jobTitle}
  ...
  const headerFieldOrder = visibleFields.filter((key) => key !== "aboutMe");
  ...
  return (
    <div className="resume-scalable flex flex-col gap-4 bg-white">
      ...
      >
        <SortableGroup
          dndId="classic-mobile-fields"
          ids={headerFieldOrder}
          onReorder={(order) =>
            onReorderFields([
              ...order,

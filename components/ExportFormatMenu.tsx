 ...
 * `EmailButton` should act on (PDF or plain text) — a single shared choice
 * governing both actions, rather than each button having its own picker.
 * Modeled directly on `LanguageSelect.tsx`'s hand-rolled daisyUI dropdown
 * (`dropdown` + `dropdown-content menu` + `active` class on the selected
 * item, same Escape-to-blur handling) so it matches the app's existing
 * dropdown convention instead of introducing a new one.
 */
    ...
    <div className="dropdown" onKeyDown={handleKeyDown}>
      ...
      >
        <span className="truncate">
          {t(format === "txt" ? "buttons.formatTxt" : "buttons.formatPdf")}
        </span>
